...
function mergeRemoteData(remote) {
  ...
  tables.forEach((table) => {
    ...
    if (table === "applications") data[table] = uniqueApplicationRows(data[table]);
  });
...
function baseStock(product) {
  if (product.quantity !== "" && product.quantity != null) return parseDecimal(product.quantity);
  if (product.calculatedStock !== "" && product.calculatedStock != null) return parseDecimal(product.calculatedStock);
...
function uniqueApplicationRows(rows = data.applications) {
  ...
  rows.forEach((row) => {
    const key = [
      ...
      normalizeName(row.productId || ""),
      normalizeName(row.productName || ""),
      String(row.lotId || "").trim().toLowerCase(),
      String(row.date || "").trim(),
      parseDecimal(row.dose).toFixed(6),
      parseDecimal(row.hectares).toFixed(6),
      applicationQuantity(row).toFixed(6)
    ].join("|");
...
function stockForProduct(product) {
  ...
  const movements = uniqueApplicationRows(data.applications).filter((application) => productMatchesApplication(product, application));
...
function fillSelects() {
  ...
  document.querySelectorAll('select[name="productId"]').forEach((select) => {
    ...
    select.innerHTML = [
      ...
      ...data.products.map((product) => `<option value="${product.id}">${product.name} (${product.unit})</option>`),
...
function toggleManualProductInput(form = document.querySelector("#applicationForm")) {
  ...
  form.querySelector(".manual-product")?.classList.toggle("hidden-panel", !manual);
  if (form.elements.manualProductName) form.elements.manualProductName.required = manual;
}

function getOrCreateManualProduct(name, lotId = "") {
  const cleanName = String(name || "").trim();
  if (!cleanName) return null;
  const existing = data.products.find((product) => normalizeName(product.name) === normalizeName(cleanName));
...
function syncApplicationDoseQuantity(form = document.querySelector("#applicationForm")) {
  if (!form) return;
  if (form.dataset.quantitySource === "quantity") syncApplicationDoseFromQuantity(form);
  else syncApplicationQuantityFromDose(form);
}
...
function renderOrders() {
  ...
  document.querySelectorAll("[data-order-status]").forEach((button) => {
    button.addEventListener("click", (event) => {
      event.stopPropagation();
      updateOrderStatus(button.dataset.orderStatus, button.dataset.nextStatus);
    });
  });
...
function editApplication(key) {
  ...
  const productExists = data.products.some((product) => product.id === row.productId);
  form.elements.productId.value = productExists ? row.productId : "__manual__";
  if (form.elements.manualProductName) form.elements.manualProductName.value = productExists ? "" : (row.productName || row.productId || "");
  form.elements.dose.value = row.dose || "";
  if (form.elements.usedQuantity) form.elements.usedQuantity.value = row.usedQuantity || "";
...
function bindForms() {
  ...
  applicationForm.elements.usedQuantity?.addEventListener("input", () => {
    applicationForm.dataset.quantitySource = "quantity";
    syncApplicationDoseFromQuantity(applicationForm);
  });
  applicationForm.elements.hectares.addEventListener("input", () => syncApplicationDoseQuantity(applicationForm));
  ...
  document.querySelector("#applicationForm").addEventListener("submit", (event) => {
    ...
    if (keepContext) {
      const kept = {
        ...
        quantitySource: event.currentTarget.dataset.quantitySource || "dose"
      };
      resetForm(event.currentTarget);
      ...
      if (event.currentTarget.elements.usedQuantity) event.currentTarget.elements.usedQuantity.value = "";
      event.currentTarget.dataset.quantitySource = kept.quantitySource;
