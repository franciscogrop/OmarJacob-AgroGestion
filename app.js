...
function populateOrderExtraFilters() {
  ...
  if (contractorSelect) {
    const contractors = Array.from(data.orders.reduce((map, order) => {
      ...
      return map;
    }, new Map()).values()).sort((a, b) => a.localeCompare(b, "es", { sensitivity: "base", numeric: true }));
...
async function copyApplicationOrder(applicationId) {
  ...
  try {
    ...
    showToast("No se pudo copiar autom?ticamente");
  }
}
...
function ensureCompleteReceiptsTable() {
  ...
  panel.innerHTML = `
    <div class="panel-header">
      ...
      <span class="panel-note">Agrupados por número, fecha, proveedor y depósito</span>
    </div>
    <div class="table-wrap">
      <table>
        <thead>
